...
function RequestComponent({ bookingID, sid, name, rollno, hostel, checkin, checkout, type1, type2, guest1Name, guest1Contact, guest1Email, guest2Name, guest2Contact, guest2Email,refreshKey,setRefreshKey}: RequestComponentProps) {
    ...
    const handleConfirm = async (review_Status: string) => {
        const token = await getAccessTokenSilently();
        const message = messageInput.current?.value;
        const bookingId = bookingID;
        const sup_id = sid;
        let reviewStatus = (review_Status === "Approve") ? "approved" : "rejected";

        console.log("API CALLED FOR REVIEW");
        console.log("data being sent",bookingId, sid, reviewStatus, message);

        try {
            const response = await axios.post(`${process.env.REACT_APP_API_URL}/users/reviewBooking`, {
                bookingId,
                sid,
                reviewStatus,
                message
        ...
        <div className=" w-full border-2 rounded-md shadow-sm flex flex-col p-4 font-inter mb-4">
            ...
            {showDetails &&
                <div className="mt-3">
                    ...
                    <div className={`flex flex-row mt-3 text-sm font-inter justify-between sm:w-1/2 text-gray-700 transition-height duration-1000 ${showDetails ? "max-h-full" : "max-h-0"
                        }`}>
